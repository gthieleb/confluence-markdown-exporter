...
class Exporter:
    ...
    def __sanitize_filename(self, document_name_raw):
        ...
        for invalid in ["..", "/"]:
            if invalid in document_name:
                print(
                    'Dangerous page title: "{}", "{}" found, replacing it with "_"'.format(
    ...
    def __dump_page(self, src_id, parents):
        ...
        if not self.__no_attach:
            ret = self.__confluence.get_attachments_from_content(
                page_id, start=0, limit=500, expand=None, filename=None, media_type=None
            )
            for i in ret["results"]:
                ...
                att_filename = os.path.join(
                    page_output_dir, ATTACHMENT_FOLDER_NAME, att_sanitized_name
                )
                ...
                r = requests.get(
                    att_url, auth=(self.__username, self.__token), stream=True
                )
    ...
    def dump(self):
        ret = self.__confluence.get_all_spaces(
            start=0, limit=500, expand="description.plain,homepage"
        )
        if ret["size"] == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--space", type=str, required=False, default=None, help="Spaces to export"
    )
    parser.add_argument(
        "--skip-attachments",
        action="store_true",
        ...
        required=False,
        default=False,
        help="Skip fetching attachments",
    )
    parser.add_argument(
        "--no-fetch",
        action="store_true",
